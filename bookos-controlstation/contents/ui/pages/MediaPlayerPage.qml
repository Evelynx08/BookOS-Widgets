...
PageTemplate {
    ...
    readonly property int length: mpris2Model.currentPlayer?.length ?? 0
    readonly property bool canSeek: mpris2Model.currentPlayer?.canSeek ?? false
    readonly property string playerIcon: mpris2Model.currentPlayer?.iconName ?? ""
    function position() { return mpris2Model.currentPlayer?.position ?? 0; }
    function setPosition(us) { if (mpris2Model.currentPlayer) mpris2Model.currentPlayer.position = us; }
