...
PlasmoidItem {
    ...
    // ── BookOS (One UI) design tokens ──────────────────────────────
    // Active = saturated blue, Inactive = soft blue. Tiles = neutral grey at 80%.
    property color bookosActive: "#4184FF"
    property color bookosInactive: "#AECAFF"
    // General popup background: pure white / near-pure black
    property color bookosBg: isDarkTheme ? "#0A0A0A" : "#FFFFFF"
    // Tile/box background (cards holding buttons): DFDFDF @ 80% on light,
    // a lifted dark grey @ 80% on dark.
    property color bookosTile: isDarkTheme ? Qt.rgba(0.16, 0.16, 0.16, 0.80)
                                           : Qt.rgba(0.874, 0.874, 0.874, 0.80)
    // Slider trough inside a tile (lighter than the tile itself)
    property color bookosTrough: isDarkTheme ? Qt.rgba(1, 1, 1, 0.10)
                                             : Qt.rgba(1, 1, 1, 0.85)
    // Radii / sizes from the spec
    property int bookosRadius: 27
    property int bookosSolitaryHeight: 74 * scale
    property int bookosCardHeight: 142 * scale
