...
Kirigami.FormLayout {
    ...
    TextField { id: scrollModeHolder; visible: false; text: "paged" }
    ...
    ComboBox {
        id: scrollModeCombo
        Kirigami.FormData.label: i18n("Scroll mode:")
        model: ["paged", "continuous"]
        property var labels: ({ "paged": i18n("Paginated (snap to page)"),
                                 "continuous": i18n("Continuous (free scroll)") })
        displayText: labels[model[currentIndex]] || ""
        delegate: ItemDelegate {
            width: scrollModeCombo.width
            required property var modelData
            text: scrollModeCombo.labels[modelData] || modelData
            highlighted: scrollModeCombo.highlightedIndex === index
        }
        currentIndex: Math.max(0, model.indexOf(scrollModeHolder.text))
        onActivated: scrollModeHolder.text = model[currentIndex]
    }
